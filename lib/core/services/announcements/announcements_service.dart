...
class AnnouncementsService {
  ...
  Future<VersionCheckResult> versionCheck({required String appVersion}) async {
    final uri = Uri.parse(
      '$centralizedBaseUrl/announcements/version-check?appVersion=$appVersion',
    );
    ...
    if (hasToken) {
      try {
        final authUri = Uri.parse(
          '$centralizedBaseUrl/announcements/me/active?appVersion=$appVersion',
        );
        body = await _getJson(authUri, auth: true);
      } catch (_) {
        final publicUri = Uri.parse('$centralizedBaseUrl/announcements/active');
  ...
  Future<void> acknowledge(String announcementId) async {
    ...
    final uri = Uri.parse(
      '$centralizedBaseUrl/announcements/${announcementId.trim()}/ack',
    );
