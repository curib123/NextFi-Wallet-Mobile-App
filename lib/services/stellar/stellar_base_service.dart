...
abstract class StellarBaseService {
  ...
  Network get network =>
      isTestnet ? Network.TESTNET : Network.PUBLIC;
  ...
  String? get _qnBase =>
      isTestnet ? quickNodeUrlTestnet : quickNodeUrlMainnet;
  ...
  String getUserFriendlyTxError(
      String code, List<String>? ops) {
    switch (code) {
  ...
  Never failSubmit(
    ...
    final code =
        res.extras?.resultCodes?.transactionResultCode ??
            'unknown';

    final ops =
        res.extras?.resultCodes?.operationsResultCodes;

    final hash = res.hash;

    final userMessage =
    getUserFriendlyTxError(code, ops?.cast<String>());
    ...
    final advice =
    getTxErrorAdvice(code, ops?.cast<String>());
  ...
  Future<dynamic> getWithFallback(
    ...
    final primary = Uri.parse('$horizonBase$path')
        .replace(queryParameters: query);

    try {
      final r =
      await sdk.httpClient.get(primary).timeout(timeout);
  ...
  Stream<T> sseWithFallback<T>(
      Stream<T> Function(StellarSDK s) build) {
  ...
  int toStroops(double amount) =>
      (amount * 1e7).round();

  double fromStroops(int stroops) =>
      stroops / 1e7;
